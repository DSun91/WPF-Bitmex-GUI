    ...
    public class BinanceAPI : GeneralExchangeAPI
    {
        ...
        override protected void ProcessPriceResponseWss(string response)
        {
            ...
            var priceData = new CandlestickData
            {
                ...
                Width = OHLCandlestickChart.candleWidth
            };
        ...
        public override void ProcessResponseRest(string response, ObservableCollection<CandleStickViewModel> PriceData, Dictionary<string, CandlestickData> _priceDataDictionary)
        {
            ...
            foreach (var kline in klines)
            {
                ...
                InitData.Width = OHLCandlestickChart.candleWidth;
                // Store the prices in a list 

                // Add to the SortedDictionary
                if (!PriceData.Any(x => x.Candlestick.Timestamp == InitData.Timestamp))
                {
                    InitData.Posx = OHLCandlestickChart.CandlesInterspace * PriceData.Count;
                    PriceData.Add(new CandleStickViewModel( InitData));
