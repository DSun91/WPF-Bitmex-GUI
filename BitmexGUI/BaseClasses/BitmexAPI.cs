...
namespace BitmexGUI.BaseClasses
{
    public class BitmexAPI : GeneralExchangeAPI
    {
        ...
        public class BitmexNewWSSConnectionRequestParams : IWssConnectionArgs
        {
            public string WebsocketName { get; set; }
            public string BaseWss { get; set; }
        ...
        #region AUTH_CONNTECTIONS_SECTION
        ...
        protected override async Task ConnectWebSocketEndpoint(ClientWebSocket WebSocket, IWssConnectionArgs ConnectionsArgs)
        {
            var Params = ConnectionsArgs as BitmexNewWSSConnectionRequestParams;
             
            if (WebSocket.State != WebSocketState.Connecting && WebSocket.State != WebSocketState.Open && WebSocket.State != WebSocketState.Closed)
            {
                await WebSocket.ConnectAsync(new Uri(Params.BaseWss), ConnectionTokenSource.Token);
                WebSocketManager.Instance.AddWebSocket(Params.WebsocketName, WebSocket);
                if (WebSocket.State == WebSocketState.Open)
                {
                    var auth_message = new
                    {
                        op = "authKeyExpires",
                        args = new object[] { ApiID, Params.expires, Params.signature }
                    };

                    string dataJson = JsonConvert.SerializeObject(auth_message);
                    await SendMessageAsync(dataJson, WebSocket);


                    var subscribe_message = new
                    {
                        op = "subscribe",
                        args = new string[] { Params.channel }

                    };

                    string dataJsonSub = JsonConvert.SerializeObject(subscribe_message);
                    await SendMessageAsync(dataJsonSub, WebSocket);
                }
        ...
        public async void GetOrdersWSS()
        {
            ...
            BitmexNewWSSConnectionRequestParams ConnectionArgs= new BitmexNewWSSConnectionRequestParams
            {
                WebsocketName = "OrdersFeed",
                BaseWss = BaseWss,
                expires = expires,
                signature = signature,
                channel= "order"
            ...
            await ConnectWebSocketEndpoint(BitmexHttpClientOrdersWSS, ConnectionArgs);
             
            if (BitmexHttpClientOrdersWSS.State == WebSocketState.Open)
            {
                await foreach (var message in FetchWebsocketResponse(BitmexHttpClientOrdersWSS))
                {
                    if (!string.IsNullOrEmpty(message))
                    {
                        ProcessResponseOrder(message);
                    }
                }
            }
        ...
        public async void GetPositionWSS()
        {
            ...
            try
            {
                ...
                await ConnectWebSocketEndpoint(BitmexHttpClientPositionsWSS, ConnectionArgs);

                 
                if (BitmexHttpClientPositionsWSS.State == WebSocketState.Open)
                {
                    await foreach (var message in FetchWebsocketResponse(BitmexHttpClientPositionsWSS))
                    {
                        if (!string.IsNullOrEmpty(message))
                        {
                            ProcessResponsePosition(message);
                        }
                    }
                }
        ...
        #region LIVE_PRICE_FEED
        public override async void GetPriceWSS(string WebsocketName)
        {
            ClientWebSocket HttpClientPriceWSS = new ClientWebSocket();


            if (HttpClientPriceWSS.State != WebSocketState.Connecting && HttpClientPriceWSS.State != WebSocketState.Open)
            {
                await HttpClientPriceWSS.ConnectAsync(new Uri(UrlWss), CancellationToken.None);
                WebSocketManager.Instance.AddWebSocket(WebsocketName, HttpClientPriceWSS);
            }


            await foreach (var message in FetchWebsocketResponse(HttpClientPriceWSS))
            {
                if (!string.IsNullOrEmpty(message))
                {
                    ProcessPriceResponseWss(message);
                }
            }
